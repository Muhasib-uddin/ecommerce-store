...
import { realRegister } from "../../../helpers/real_backend_helper"
...
function* registerUser({ payload: { user } }) {
  ...
  try {
    ...
    if (import.meta.env.VITE_APP_DEFAULTAUTH === "real") {
      const response = yield call(realRegister, {
        email: user.email,
        password: user.password,
        firstName: user.firstName || user.username,
        lastName: user.lastName || "",
      })
      const userData = response.data ? response.data.user || response.data : response
      yield put(registerUserSuccessful(userData))
